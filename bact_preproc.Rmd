...
```{r}
ps.contam <- prune_taxa(contamdf.prev01$contaminant, ps.bact)
ps.contam
plot_bar(ps.contam, x = "sample.type", y = "Abundance") + #fill = "genus"
  facet_wrap(. ~ Phylum)

check_taxa <- subset_taxa(ps.bact, Phylum == "Proteobacteria") #Pasteurellaceae #Oxalofunteraceae #Pseudomonadaceae
plot_bar(check_taxa, x = "sample.type", y = "Abundance", fill = "Genus")

ps.bact1 <- prune_taxa(!contamdf.prev01$contaminant, ps.bact)
ps.bact1
```

```{r}
ps.pa <- transform_sample_counts(ps.bact, function(abund) 1*(abund>0))
...
```{r}
ps.bact.neg <- subset_samples(ps.bact, sample.type == "NEG" ) #6 samples
...
prevdf = apply(X = otu_table(ps.bact),
                 MARGIN = ifelse(taxa_are_rows(ps.bact), yes = 1, no = 2),
                 FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf = data.frame(Prevalence = prevdf,
                      TotalAbundance = taxa_sums(ps.bact),
                      tax_table(ps.bact))
...
ps.nosingletons <- prune_taxa(taxa_sums(ps.bact) > 1, ps.bact)
summarize_phyloseq(ps.nosingletons)
length(which(taxa_sums(ps.nosingletons) <= 1))
ps.nosingletons
ps.bact

#do on the actual ps.bact
...
per time point T2
```{r}
ps.bact2 <- ps.bact %>% subset_samples(time.point == "T2")
T2.readcount <- as.data.frame (readcount(ps.bact2))
asv_df2 <- t(otu_table(ps.bact2))
T2.readcount$asvcount <- colSums(asv_df2 != 0)

famfac2 = factor(tax_table(ps.bact2)[, "Family"])
# Try as before
famtab2 = apply(otu_table(ps.bact2), MARGIN = 1, function(x) {
    tapply(x, INDEX = famfac, FUN = sum, na.rm = TRUE, simplify = TRUE)})

T2.readcount$famcount <- colSums(famtab2 != 0)
write.csv(ASV.reads, "./Bacteria/reads.asv.families per sample-T2.csv", row.names=FALSE)
```


per time point T3
```{r}
ps.bact3 <- ps.bact %>% subset_samples(time.point == "T3")
T3.readcount <- as.data.frame (readcount(ps.bact3))
asv_df3 <- t(otu_table(ps.bact3))
T3.readcount$asvcount <- colSums(asv_df3 != 0)

famfac3 = factor(tax_table(ps.bact3)[, "Family"])
# Try as before
famtab3 = apply(otu_table(ps.bact3), MARGIN = 1, function(x) {
    tapply(x, INDEX = famfac, FUN = sum, na.rm = TRUE, simplify = TRUE)})

T3.readcount$famcount <- colSums(famtab3 != 0)
write.csv(ASV.reads, "./Bacteria/reads.asv.families per sample-T3.csv", row.names=FALSE) 
```

prune and check
```{r}
sample_sums(ps.bact) > 20000 %>% sum
ps.bact %>% subset_samples(time.point == "T1") %>% sample_sums > 20000 %>% sum 

ps.bactpruned10000 <- prune_samples(sample_sums(ps.bact) > 10000, ps.bact)
ps.bactpruned10000 #685 samples removed 6 samples

ps.bact <- prune_samples(sample_sums(ps.bact) > 10000, ps.bact)
ps.bact #685 samples removed 5 samples
```
...
```{r}
library(microbiome)
write.table(ps.bact@tax_table@.Data, file = "./Bacteria/bact_tax_table.txt", sep = "\t")
write.table(ps.bact@sam_data@.Data, file = "./Bacteria/bact_metadata_table.txt", sep = "\t")
write.table(ps.bact@otu_table@.Data, file = "./Bacteria/bact_otu_table.txt", sep = "\t")

#write_phyloseq(physeq.fungi, type = "OTU", path = "./")
#write_phyloseq(ps.bact, type = "METADATA", path = "./bact/")
```
